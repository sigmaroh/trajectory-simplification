// ─── algorithm colours ────────────────────────────────────────────────────────
const ALGO_COLOR = {
  original:'#6b7280', dp:'#3b82f6', vw:'#f97316', squish:'#a855f7',
  rw:'#ef4444', greedy_policy:'#7c3aed', rl_dqn:'#94a3b8',
  proposed:'#111827', us:'#14b8a6', at:'#64748b',
};
const ALGO_LABEL = {
  ...
  try {
    ...
    if (match) {
      const fields = [
        ...
        ['Fréchet (m)',      'frechet_distance'],
        ['APTE (m)',         'average_pte'],
        ['PED (m)',          'ped'],
        ['SED (m)',          'sed'],
        ['DAD (°)',          'dad'],
        ['SAD (m/s)',        'sad'],
        ['Turn Pres.',       'turn_preservation'],
        ['Stop Pres.',       'stop_preservation'],
      ...
      fields.forEach(([lbl,k]) => {
        ...
        html += `<tr><td style="color:#64748b;padding:2px 0">${lbl}</td><td>${disp}</td></tr>`;
      });
...
async function loadOsmComparison() {
  ...
  for (const algo of algos) {
    try {
      ...
      const line    = L.polyline(latlngs, {
        color, weight: algo === 'original' ? 3 : 2,
        opacity: algo === 'original' ? 0.85 : 0.8,
        dashArray: algo === 'original' ? null : (algo === 'proposed' ? '6 4' : null),
      }).bindTooltip(`${algoLabel(algo)} (${cr}×) — ${pts.length} pts`).addTo(map);
      activeLeafletLayers.push(line);

      // start/end markers
      [L.circleMarker(latlngs[0], { radius:4, color:'#fff', weight:1, fillColor:color, fillOpacity:1 }),
       L.circleMarker(latlngs[latlngs.length-1], { radius:4, color:'#fff', weight:1, fillColor:'#111', fillOpacity:0.8 })]
        .forEach(m => { m.addTo(map); activeLeafletLayers.push(m); });
  ...
  osmLayerReg.forEach(reg => {
    ...
    row.append(cb, sw, txt); layerList.appendChild(row);
  });
...
function refreshOsmMetrics() {
  ...
  osmLayerReg.forEach(reg => {
    ...
    tr.innerHTML = `
      ...
      <td>${fmt(m.sad)}</td>
      <td>${fmt(m.issd)}</td>
      <td>${pct(m.turn_preservation)}</td>
      <td>${pct(m.stop_preservation)}</td>`;
    tbody.appendChild(tr);
  });
  if (!tbody.children.length) {
    tbody.innerHTML = '<tr><td colspan="12" style="color:#94a3b8;padding:.5rem">No stored metrics — run experiments first.</td></tr>';
  }
}
